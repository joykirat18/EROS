...
class BertNER(BertPreTrainedModel):
    ...
    def forward(self,loadall, all_span_lens, all_span_idxs_ltoken, input_ids, token_type_ids=None, attention_mask=None):
        ...
        all_span_idxs_ltoken = all_span_idxs_ltoken.to(self.args.device)
        all_span_lens = all_span_lens.to(self.args.device)
        # print(f"cuda of input_ids {input_ids.get_device()}")
        # print(f"bert model device: {self.bert.get_device()}")
        bert_outputs = self.bert(input_ids, token_type_ids=token_type_ids, attention_mask=attention_mask)
        sequence_heatmap = bert_outputs[0]  # [batch, seq_len, hidden]
        # print(f"cuda of sequence_heatmap {sequence_heatmap.get_device()}")
        ...
        # all_span_feature_rep = self.span_embedding.getFeature_output(all_span_rep)
        all_span_rep, all_span_feature_rep = self.span_embedding(all_span_rep)  # (batch,n_span,n_class)
            

        return all_span_rep, all_span_feature_rep
