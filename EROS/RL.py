...
def getReward(model, args,generated_summary, originalPolicy, goldSummary):
    ...
    try:
        # import math
        R3 = (total_reward - 0.2 * abs(sum(generated_counter.values()) - total_reward)) / max_reward
    ...
    R2 = 1 - (abs(len(generated_summary) - len(goldSummary))/ max(len(goldSummary),len(generated_summary)))
    print(R1, R2, R3)
    reward = (R1 + R3)
    # reward = (R2)min
